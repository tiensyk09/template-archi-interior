...
function HeroBlock({ configs = {} }) {
  return (
    <div style={{
      background: 'linear-gradient(135deg, #b08d57 0%, #211d18 100%)',
      color: '#ffffff',
      borderRadius: '16px',
      padding: '56px 32px',
      textAlign: 'center',
      marginBottom: '32px',
      boxShadow: '0 4px 24px rgba(33, 29, 24, 0.15)',
      position: 'relative',
      overflow: 'hidden'
    }}>
      ...
      <p style={{ fontSize: '16px', lineHeight: 1.7, opacity: 0.9, maxWidth: '750px', margin: '0 auto 28px', color: '#f4efe7' }}>
        {configs.description || ''}
      </p>
      {configs.buttonText && configs.buttonLink && (
        <Link
          ...
          style={{
            ...
            textDecoration: 'none',
            transition: 'all 0.2s',
            boxShadow: '0 4px 14px rgba(176, 141, 87, 0.4)'
          }}
          onMouseEnter={(e) => { e.currentTarget.style.transform = 'translateY(-2px)'; e.currentTarget.style.boxShadow = '0 6px 18px rgba(176, 141, 87, 0.5)'; }}
          onMouseLeave={(e) => { e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.boxShadow = '0 4px 14px rgba(176, 141, 87, 0.4)'; }}
        >
          {configs.buttonText}
        </Link>
      )}
    ...
    <div style={{ padding: '32px 0', marginBottom: '32px' }}>
      {configs.title && (
        <div style={{ textAlign: 'center', marginBottom: '36px' }}>
          {configs.tag && (
            <span style={{ fontSize: '12px', fontWeight: 800, color: '#b08d57', textTransform: 'uppercase', letterSpacing: '0.08em', display: 'block', marginBottom: '8px' }}>
              {configs.tag}
            </span>
          )}
          <h3 style={{ fontSize: '26px', fontWeight: 800, color: '#211d18' }}>{configs.title}</h3>
          {configs.description && (
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '24px' }}>
        {items.map((item, idx) => (
          <div key={idx} style={{ background: '#ffffff', border: '1px solid #e5e7eb', borderRadius: '16px', padding: '24px', boxShadow: '0 4px 16px rgba(0,0,0,0.02)', display: 'flex', gap: '16px', alignItems: 'flex-start' }}>
            <div style={{ background: '#f4efe7', color: '#b08d57', width: '48px', height: '48px', borderRadius: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '20px', flexShrink: 0 }}>
              {idx === 0 ? '🏠' : idx === 1 ? '✨' : '💖'}
            </div>
            <div>
              <h4 style={{ fontSize: '16px', fontWeight: 700, color: '#211d18', margin: '0 0 8px 0' }}>{item.title}</h4>
...
const DEFAULT_CATEGORIES = [
  { title: 'Thiết kế kiến trúc', sub: '8 gói dịch vụ', img: 'https://images.unsplash.com/photo-1503387762-592deb58ef4e?auto=format&fit=crop&w=300&q=80', icon: '📐' },
  { title: 'Thiết kế nội thất', sub: '10 gói dịch vụ', img: 'https://images.unsplash.com/photo-1600607687939-ce8a6c25118c?auto=format&fit=crop&w=300&q=80', icon: '🛋️' },
  { title: 'Xây dựng phần thô', sub: '5 gói dịch vụ', img: 'https://images.unsplash.com/photo-1541888946425-d81bb19240f5?auto=format&fit=crop&w=300&q=80', icon: '🏗️' },
  { title: 'Thi công nội thất', sub: '12 gói dịch vụ', img: 'https://images.unsplash.com/photo-1586023492125-27b2c045efd7?auto=format&fit=crop&w=300&q=80', icon: '🔧' },
  { title: 'Cải tạo & sửa chữa', sub: '6 gói dịch vụ', img: 'https://images.unsplash.com/photo-1615873968403-89e068629265?auto=format&fit=crop&w=300&q=80', icon: '🔨' },
  { title: 'Biệt thự', sub: '15 dự án', img: 'https://images.unsplash.com/photo-1600585154340-be6161a56a0c?auto=format&fit=crop&w=300&q=80', icon: '🏡' },
  { title: 'Căn hộ chung cư', sub: '32 dự án', img: 'https://images.unsplash.com/photo-1600210492486-724fe5c67fb0?auto=format&fit=crop&w=300&q=80', icon: '🏢' },
  { title: 'Nhà phố', sub: '28 dự án', img: 'https://images.unsplash.com/photo-1600566753190-17f0baa2a6c3?auto=format&fit=crop&w=300&q=80', icon: '🏘️' },
  { title: 'Văn phòng', sub: '12 dự án', img: 'https://images.unsplash.com/photo-1497366754035-f200968a6e72?auto=format&fit=crop&w=300&q=80', icon: '💼' },
  { title: 'Nhà bếp', sub: '9 mẫu thiết kế', img: 'https://images.unsplash.com/photo-1556912173-3bb406ef7e77?auto=format&fit=crop&w=300&q=80', icon: '🍳' },
  { title: 'Phòng ngủ', sub: '14 mẫu thiết kế', img: 'https://images.unsplash.com/photo-1600121848594-d8644e57abab?auto=format&fit=crop&w=300&q=80', icon: '🛏️' },
  { title: 'Không gian xanh', sub: '7 mẫu thiết kế', img: 'https://images.unsplash.com/photo-1618221195710-dd6b41faaea6?auto=format&fit=crop&w=300&q=80', icon: '🌿' },
];
...
const DEFAULT_HEALTH_CHECKS = [
  { title: 'Ước tính chi phí xây nhà trọn gói', desc: 'Nhập diện tích và số tầng để nhận báo giá sơ bộ', icon: '🏗️', action: 'Bắt đầu' },
  { title: 'Ước tính chi phí nội thất căn hộ', desc: 'Chọn phong cách và diện tích để dự toán ngân sách', icon: '🛋️', action: 'Bắt đầu' },
  { title: 'Trắc nghiệm phong cách thiết kế của bạn', desc: 'Khám phá phong cách nội thất phù hợp với gu thẩm mỹ', icon: '🎨', action: 'Bắt đầu' },
];
      ...
      <div className="lc-section">
        <div className="lc-healthcheck-section">
          <div className="lc-healthcheck-left">
            <h2 className="lc-healthcheck-title">{configs.title || 'Công cụ dự toán chi phí'}</h2>
            <p className="lc-healthcheck-desc">{configs.description || 'Ước tính nhanh ngân sách cho công trình của bạn chỉ trong 1 phút!'}</p>
...
const DEFAULT_AUDIENCES = [
  {
    ...
    bgColor: 'linear-gradient(135deg, #f4efe7, #e8ddc9)',
    items: ['Thiết kế kiến trúc sân vườn', 'Nội thất gỗ tự nhiên cao cấp', 'Hệ smarthome toàn diện', 'Hồ bơi & tiểu cảnh'],
  },
  {
    name: 'CĂN HỘ CHUNG CƯ',
    img: 'https://images.unsplash.com/photo-1600607687939-ce8a6c25118c?auto=format&fit=crop&w=400&q=80',
    bgColor: 'linear-gradient(135deg, #ece7de, #ddd3c2)',
    items: ['Tối ưu diện tích nhỏ', 'Nội thất đa năng thông minh', 'Phong cách hiện đại tối giản', 'Thi công nhanh 30-45 ngày'],
  },
  {
    name: 'NHÀ PHỐ',
    img: 'https://images.unsplash.com/photo-1600566753190-17f0baa2a6c3?auto=format&fit=crop&w=400&q=80',
    bgColor: 'linear-gradient(135deg, #f0ebe2, #e0d6c5)',
    items: ['Xây mới trọn gói chìa khóa trao tay', 'Thiết kế lấy sáng giếng trời', 'Nâng tầng, mở rộng không gian', 'Chống thấm, chống nóng'],
  },
  {
    name: 'VĂN PHÒNG',
    img: 'https://images.unsplash.com/photo-1497366754035-f200968a6e72?auto=format&fit=crop&w=400&q=80',
    bgColor: 'linear-gradient(135deg, #eae5dc, #d9cfbe)',
    items: ['Không gian mở sáng tạo', 'Phòng họp cách âm hiện đại', 'Khu pantry & thư giãn', 'Thi công ngoài giờ hành chính'],
  },
];
    ...
    <div className="lc-section-bg-light">
      <div className="lc-section">
        <div className="lc-section-header">
          <div className="lc-section-title-row">
            <span className="lc-section-icon">👥</span>
            <h2 className="lc-section-title">{configs.title || 'Giải pháp theo loại công trình'}</h2>
          </div>
        </div>
              ...
              {listPosts.map((post, i) => (
                <div key={i} className="lc-blog-row">
                  ...
                  <div className="lc-blog-row-info">
                    ...
                    <span>{post.author_name || 'ARCHI'} · {new Date(post.created_at).toLocaleDateString('vi-VN')}</span>
                  </div>
                </div>
              ))}
            </div>
...
const DEFAULT_BRANDS = [
  { name: 'An Cường', logoImg: 'https://images.unsplash.com/photo-1586023492125-27b2c045efd7?auto=format&fit=crop&w=200&q=80', productImg: 'https://images.unsplash.com/photo-1586023492125-27b2c045efd7?auto=format&fit=crop&w=300&q=80', discount: 'Gỗ công nghiệp cao cấp' },
  { name: 'Hafele', logoImg: 'https://images.unsplash.com/photo-1556912173-3bb406ef7e77?auto=format&fit=crop&w=200&q=80', productImg: 'https://images.unsplash.com/photo-1556912173-3bb406ef7e77?auto=format&fit=crop&w=300&q=80', discount: 'Phụ kiện Đức chính hãng' },
  { name: 'Dulux', logoImg: 'https://images.unsplash.com/photo-1615873968403-89e068629265?auto=format&fit=crop&w=200&q=80', productImg: 'https://images.unsplash.com/photo-1615873968403-89e068629265?auto=format&fit=crop&w=300&q=80', discount: 'Sơn nội ngoại thất' },
  { name: 'Viglacera', logoImg: 'https://images.unsplash.com/photo-1541888946425-d81bb19240f5?auto=format&fit=crop&w=200&q=80', productImg: 'https://images.unsplash.com/photo-1541888946425-d81bb19240f5?auto=format&fit=crop&w=300&q=80', discount: 'Gạch ốp lát, thiết bị vệ sinh' },
  { name: 'Blum', logoImg: 'https://images.unsplash.com/photo-1600121848594-d8644e57abab?auto=format&fit=crop&w=200&q=80', productImg: 'https://images.unsplash.com/photo-1600121848594-d8644e57abab?auto=format&fit=crop&w=300&q=80', discount: 'Ray trượt, bản lề Áo' },
];
