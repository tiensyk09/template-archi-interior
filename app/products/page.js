...
export async function generateMetadata({ searchParams }) {
  ...
  let title = 'Tất cả sản phẩm | ARCHI';
  if (categorySlug) {
    const cats = await query('SELECT name FROM shop_categories WHERE slug = ?', [categorySlug]);
    if (cats.length > 0) {
      title = `${cats[0].name} | ARCHI`;
    }
  } else if (searchQ) {
    title = `Tìm kiếm: "${searchQ}" | ARCHI`;
  }
  
  return {
    title,
    description: 'Dịch vụ thiết kế kiến trúc, nội thất, xây dựng phần thô, thi công trọn gói và cải tạo sửa chữa tại ARCHI.'
  };
}
