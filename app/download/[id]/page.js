...
export async function generateMetadata({ params }) {
  ...
  if (!decoded) return { title: 'Asset Not Found | ARCHI' };
  ...
  if (!file || file.is_public === 0) {
    return { title: 'Asset Restricted | ARCHI' };
  }

  return {
    title: `Download: ${file.name} | ARCHI`,
    description: file.description || `Download asset ${file.name} securely from ARCHI.`
  };
}
